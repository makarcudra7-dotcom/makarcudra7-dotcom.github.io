...
CANONICAL = [
    ...
    (r'\bяблок\w*\b', 'яблоко'),
    (r'\bананас\w*\b', 'ананас'),
    (r'\bпекинск\w* капуст\w*\b', 'пекинская капуста'),
    (r'\bсухар\w*\b', 'сухарики'),
    (r'\bмук\w*\b', 'мука'),
...
def ingredient_name(line: str) -> str:
    ...
    raw = re.sub(r'^(?:банка|банки|зубчик\w*|небольш\w*|средн\w*|крупн\w*|маленьк\w*|примерно|около)\s+', '', raw)
    ...
    for pattern, canonical in CANONICAL:
        if re.search(pattern, raw, re.I): return canonical
    return raw
...
def legacy_prose_lists(body: str):
    """Legacy recipes sometimes put the main ingredient list in a sentence."""
    text=clean(body)
    found=[]
    patterns=[
        r'(?:для\s+(?:основы|салата|теста|начинки|блюда)\s+)?понадоб(?:ится|ятся)\s+(.+?)(?:\.|\bзатем\b|\bпосле этого\b)',
        r'(?:берем|возьмите|нужно)\s+(.+?)(?:\.|\bзатем\b|\bпосле этого\b)'
    ]
    for pattern in patterns:
        for hit in re.finditer(pattern,text,re.I):
            chunk=hit.group(1)
            for part in re.split(r',|\s+и\s+(?=[^,]{1,45}(?:$|,))',chunk):
                part=part.strip(' ;')
                if part and len(part)<=90 and INGREDIENT_HINT_RE.search(part): found.append(part)
    return found

def inline_quantity_pairs(body: str):
    text=clean(body);found=[]
    pattern=re.compile(r'([А-Яа-яЁё][А-Яа-яЁё\s%-]{1,45}?)\s*[—–-]\s*(\d+(?:[.,]\d+)?(?:\s*[–-]\s*\d+(?:[.,]\d+)?)?\s*'+UNIT+r')',re.I)
    for m in pattern.finditer(text):
        name=re.sub(r'^.*?[.;:]\s*','',m.group(1)).strip();line=f'{name} — {m.group(2)}'
        if INGREDIENT_HINT_RE.search(name) and len(name)<=50:found.append(line)
    pattern2=re.compile(r'(\d+(?:[.,]\d+)?\s*'+UNIT+r'\s+[А-Яа-яЁё][А-Яа-яЁё\s-]{1,35}|\d+\s+(?:яйц\w*|яблок\w*|луковиц\w*|картофел\w*))',re.I)
    for m in pattern2.finditer(text):
        line=m.group(1).strip()
        if ingredientish(line):found.append(line)
    return found

def extract_ingredients(doc: str):
    body_match=BODY_RE.search(doc);body=body_match.group(1) if body_match else doc;rows=[]
    section=heading_section(body)
    if section:
        for lst in LIST_RE.findall(section): rows.extend(x for x in (clean(v) for v in LI_RE.findall(lst)) if x and ingredientish(x))
        if not rows: rows.extend(prose_ingredients(section))
    # Always merge prose lists because legacy recipes may have main ingredients in prose and sauce in a list.
    rows.extend(legacy_prose_lists(body))
    ranked=list_candidates(body)
    if len(rows)<4:
        for score,_,candidate in ranked[:4]:
            if score>=4 and len(candidate)>=2: rows.extend(candidate)
    else:
        # Include clearly ingredient-like auxiliary lists such as sauces or toppings.
        for score,_,candidate in ranked[:3]:
            if score>=8 and len(candidate)>=3: rows.extend(candidate)
    if len(rows)<3: rows.extend(prose_ingredients(body))
    if len(rows)<3: rows.extend(inline_quantity_pairs(body))
    seen,result=set(),[]
    for row in rows:
        row=clean(row);key=row.lower()
        if row and key not in seen and ingredientish(row):seen.add(key);result.append(row)
...
def main():
    posts=json.loads(POSTS.read_text(encoding='utf-8'));recipes=[]
    for post in posts:
        if str(post.get('category','')).lower()!='рецепты' and str(post.get('type','')).lower()!='recipe':continue
        slug=post.get('slug');path=ROOT/'articles'/f'{slug}.html'
        if not slug or not path.exists():continue
        doc=path.read_text(encoding='utf-8');lines=extract_ingredients(doc);names=[]
        for line in lines:
            name=ingredient_name(line)
            if name and len(name)<=80 and name not in names:names.append(name)
        ...
        if updated!=doc:path.write_text(updated,encoding='utf-8')
    OUT.write_text(json.dumps(recipes,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print(f'Indexed {len(recipes)} recipes; with 2+ ingredients: {sum(len(x["ingredients"])>=2 for x in recipes)}')
    for x in recipes:
        if len(x['ingredients'])<2:print('WARN weak ingredient extraction:',x['slug'],x['ingredients'])
