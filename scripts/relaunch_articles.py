...
authors=''.join(f'<a class="author-card" href="/{page}"><img src="{photo}?v=20260922-2" width="320" height="320" alt="{esc(name)}" onerror="this.onerror=null;this.src=\'/assets/fallback-cover.svg\'"><h3>{esc(name)}</h3><p>{esc(role)}</p></a>' for name,(page,photo,role) in AUTHORS.items())
...
if app.exists(): app.write_text(app.read_text('utf-8').replace("'elvira.svg':'/assets/authors/elvira.jpg'","'elvira.svg':'/assets/authors/elvira.jpg'"),'utf-8')
assert len(posts)==24
