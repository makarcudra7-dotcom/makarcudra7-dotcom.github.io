(()=>{
  ...
  function resetNew(){
    ...
    ['headline','lead','seoTitle','description','slug','canonical','ogImage','source','tags','image','imageAlt'].forEach(id=>{const el=$('#'+id);if(el)el.value=''});
    Object.entries(defaults).forEach(([id,v])=>{const el=$('#'+id);if(el)el.value=v});
    if($('#coauthors'))[...$('#coauthors').options].forEach(o=>o.selected=false);
  ...
  async function cropBlob(file,w,h,q=.86){
    ...
    return new Promise((res,rej)=>canvas.toBlob(b=>b?res(b):rej(new Error('Не удалось подготовить изображение')),'image/jpeg',q));
  }
