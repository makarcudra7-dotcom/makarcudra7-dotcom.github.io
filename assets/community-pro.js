(()=>{
  ...
  function updateDirectory(authors){
    ...
    grid.querySelectorAll('.author-card').forEach(card=>{
      ...
      card.innerHTML=`<div class="author-card-top"><img src="/${esc((a.photo||'assets/fallback-cover.svg').replace(/^\//,''))}" width="112" height="112" alt="${esc(a.name)}"><div><div class="author-card-role">${esc(a.role)}</div><h3>${esc(a.name)}</h3></div></div><p class="author-card-lead">${esc(a.lead||a.bio)}</p>${topics((a.topics||[]).slice(0,5))}<div class="author-card-promise">В профиле — все публикации автора, его темы и возможность задать вопрос редакции.</div><div class="author-card-footer"><span data-author-count="${esc(a.name)}">Материалы автора</span><strong>Открыть профиль →</strong></div>`;
    })
  }
  function updateProfile(authors){
    const a=authors.find(x=>x.url===page);if(!a)return;
    const hero=$('.author-profile-hero,.author-hero');if(hero&&hero.dataset.proAuthor!==a.name){
      hero.dataset.proAuthor=a.name;const info=hero.querySelector(':scope > div');if(info){const note=info.querySelector('.photo-credit')?.outerHTML||'';info.innerHTML=`<div class="eyebrow">Автор ProVkus</div><h1>${esc(a.name)}</h1><div class="author-role">${esc(a.role)}</div>${note}<p class="author-lead">${esc(a.lead||'')}</p><p class="author-bio">${esc(a.bio||'')}</p>${topics(a.topics)}<div class="author-profile-facts"><div class="author-method"><strong>Как работает с темами</strong><span>${esc(a.method||'')}</span></div><div class="author-method"><strong>Когда к автору</strong><span>${esc(a.useful||'')}</span></div></div><a class="ask-author-link profile-ask-cta" href="#ask-author">Задать вопрос ${esc(a.dativeFirst||a.name.split(' ')[0])} ↓</a>`}const photo=hero.querySelector('.author-photo');if(photo&&a.photo)photo.src='/'+a.photo.replace(/^\//,'')
    }
    const ask=$('.ask-author-box');if(ask){const h=ask.querySelector('h2'),p=ask.querySelector(':scope > p'),form=ask.querySelector('form'),title=`Задать вопрос ${a.dativeFirst||a.name.split(' ')[0]}`;if(h&&h.textContent!==title)h.textContent=title;if(p&&p.textContent!==(a.ask||''))p.textContent=a.ask||'';if(form)form.dataset.author=a.name}
  }
  function newsletterCopy(){
    $$('.newsletter-box').forEach(box=>{const p=box.querySelector(':scope > p');if(p&&p.textContent!==NEWS_COPY)p.textContent=NEWS_COPY;const note=box.querySelector('.form-note');if(note&&note.textContent!==NEWS_NOTE)note.textContent=NEWS_NOTE})
  }
  function footer(){
    ...
    const html=`<span>© ${new Date().getFullYear()} ProVkus</span><span>Практичное медиа о еде, продуктах и доме</span><span><a href="/editorial.html">Редакция</a> · <a href="/privacy.html">Конфиденциальность</a></span>`;if(bottom.innerHTML!==html)bottom.innerHTML=html
  }
  function hideUpdated(){$$('.article-date span').forEach(x=>{if(/^\s*Обновлено/i.test(x.textContent||''))x.remove()})}
  ...
  function related(posts){
    ...
    const current=posts.find(p=>p.slug===slug());if(!current)return;const t=new Date(current.publishedAt||0).getTime();
    const prev=posts.filter(p=>p.slug!==current.slug&&!p.excludeRelated&&new Date(p.publishedAt||0).getTime()<t).sort((a,b)=>new Date(b.publishedAt||0)-new Date(a.publishedAt||0)).slice(0,3);if(!prev.length)return;
  ...
  function apply(authors){updateDirectory(authors);updateProfile(authors);newsletterCopy();footer();hideUpdated();if(window.__pvPostsPromise)window.__pvPostsPromise.then(countAuthors)}
  authorsPromise.then(authors=>{apply(authors);setTimeout(()=>apply(authors),500);setTimeout(()=>apply(authors),1500)});
  if(window.__pvPostsPromise)window.__pvPostsPromise.then(related);
  footer();hideUpdated();newsletterCopy();
  const mo=new MutationObserver(()=>{hideUpdated();newsletterCopy()});mo.observe(document.body,{childList:true,subtree:true});setTimeout(()=>mo.disconnect(),5000)
})();
