(()=>{
  ...
  function updateProfile(authors){
    ...
    const hero=$('.author-profile-hero,.author-hero');if(hero&&hero.dataset.proAuthor!==a.name){
      hero.dataset.proAuthor=a.name;const info=hero.querySelector(':scope > div');if(info){const note=info.querySelector('.photo-credit')?.outerHTML||'';info.innerHTML=`<div class="eyebrow">Автор ProVkus</div><h1>${esc(a.name)}</h1><div class="author-role">${esc(a.role)}</div>${note}<p class="author-lead">${esc(a.lead||'')}</p><p class="author-bio">${esc(a.bio||'')}</p>${topics(a.topics)}<div class="author-profile-facts"><div class="author-method"><strong>Как работает с темами</strong><span>${esc(a.method||'')}</span></div><div class="author-method"><strong>Когда к автору</strong><span>${esc(a.useful||'')}</span></div></div><a class="ask-author-link profile-ask-cta" href="#ask-author">Задать вопрос ${esc(a.dativeFirst||a.name.split(' ')[0])} ↓</a>`}const photo=hero.querySelector('.author-photo');if(photo&&a.photo)photo.src='/'+a.photo.replace(/^\//,'')+(a.photoVersion?'?v='+a.photoVersion:'')
    }
