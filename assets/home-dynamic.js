(()=>{
  ...
  const placeholder='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%2216%22 height=%229%22 viewBox=%220 0 16 9%22%3E%3Crect width=%2216%22 height=%229%22 fill=%22%23dedbd5%22/%3E%3C/svg%3E';
  const deferredImg=(p,cls='')=>`<img class="pv-defer-img${cls?' '+cls:''}" src="${placeholder}" data-src="${esc(img(p))}" loading="lazy" decoding="async" width="1600" height="900" alt="${esc(p.imageAlt||p.headline)}">`;
  function armDeferredImages(){
    const nodes=[...document.querySelectorAll('img.pv-defer-img[data-src]')];if(!nodes.length)return;
    const load=el=>{if(!el?.dataset?.src)return;el.src=el.dataset.src;delete el.dataset.src;el.classList.remove('pv-defer-img')};
    if(!('IntersectionObserver'in window)){nodes.forEach(load);return;}
    const io=new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting){load(e.target);io.unobserve(e.target)}}),{rootMargin:'320px 0px',threshold:0.01});
  ...
  const card=(p,lead=false)=>lead?`<a class="lead-card" href="${esc(href(p))}"><img fetchpriority="high" decoding="async" width="1600" height="900" src="${esc(img(p))}" alt="${esc(p.imageAlt||p.headline)}"><div class="lead-copy"><div class="eyebrow">${esc(badge(p))}</div><h1>${esc(p.headline)}</h1><p>${esc(p.description||'')}</p><div class="meta-row"><span>${esc(p.author||'')}</span><span>•</span><time datetime="${esc(p.publishedAt||'')}">${esc(dateLabel(p.publishedAt))}</time></div></div></a>`:`<a class="stack-card" href="${esc(href(p))}">${deferredImg(p)}<div class="stack-copy"><span class="badge">${esc(badge(p))}</span><h3>${esc(p.headline)}</h3><div class="story-meta"><span>${esc(p.author||'')}</span><span>•</span><time datetime="${esc(p.publishedAt||'')}">${esc(dateLabel(p.publishedAt))}</time></div></div></a>`;
  const feedCard=p=>`<a class="story-card feed-card" href="${esc(href(p))}">${deferredImg(p)}<div class="story-body"><span class="badge">${esc(badge(p))}</span><h3>${esc(p.headline)}</h3><div class="story-meta"><span>${esc(p.author||'')}</span><span>•</span><time datetime="${esc(p.publishedAt||'')}">${esc(dateLabel(p.publishedAt))}</time></div></div></a>`;
  ...
  function render(posts){
    ...
    const hero=document.querySelector('.hero .hero-grid');if(hero)hero.innerHTML=`<div class="hero-main">${card(featured,true)}${adCta()}</div><div class="hero-side">${side.map(p=>card(p,false)).join('')}</div>`;
    ensureWorldBlock();
    ...
    const section=document.querySelector('.section .container'),grid=section?.querySelector('.story-grid');if(!section||!grid){armDeferredImages();return;}
