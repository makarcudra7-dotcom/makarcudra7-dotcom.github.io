(()=>{
  ...
  const card=(p,lead=false)=>lead?`<a class="lead-card" href="${esc(href(p))}"><img fetchpriority="high" decoding="async" width="1600" height="900" src="${esc(img(p))}" alt="${esc(p.imageAlt||p.headline)}"><div class="lead-copy"><div class="eyebrow">${esc(badge(p))}</div><h1>${esc(p.headline)}</h1><p>${esc(p.description||'')}</p><div class="meta-row"><span>${esc(p.author||'')}</span><span>•</span><time datetime="${esc(p.publishedAt||'')}">${esc(dateLabel(p.publishedAt))}</time></div></div></a>`:`<a class="stack-card" href="${esc(href(p))}"><img loading="lazy" decoding="async" src="${esc(img(p))}" width="1600" height="900" alt="${esc(p.imageAlt||p.headline)}"><div class="stack-copy"><span class="badge">${esc(badge(p))}</span><h3>${esc(p.headline)}</h3><div class="story-meta"><span>${esc(p.author||'')}</span><span>•</span><time datetime="${esc(p.publishedAt||'')}">${esc(dateLabel(p.publishedAt))}</time></div></div></a>`;
