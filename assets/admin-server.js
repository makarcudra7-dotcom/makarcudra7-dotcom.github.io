(()=>{
  ...
  const savedApi=()=>{
    ...
    if(!saved||STATIC_ORIGINS.has(saved))return SERVER_API;
    return saved;
  };
  ...
  /* Static GitHub Pages cannot serve /api/publish. Remove stale same-origin values
     left by the previous CMS version and route publishing through the Vercel backend. */
  ...
  async function requestAt(base,payload,hash){
    ...
    let r;
    try{
      r=await fetch(clean(base)+'/api/publish',{
        method:'POST',
  ...
  async function serverRequest(payload){
    ...
    const preferred=apiBase();
    const candidates=[preferred,SERVER_API].map(clean).filter((v,i,a)=>v&&a.indexOf(v)===i);
    let lastNetworkError=null;

    for(const base of candidates){
      const result=await requestAt(base,payload,hash);
      if(result.networkError){lastNetworkError=result.networkError;continue}
      const {r,data}=result;
      if(r.ok){
        if(base===SERVER_API&&preferred!==SERVER_API)localStorage.removeItem(API_KEY);
        return data;
      }
      if(r.status===401)throw new Error('Сервер публикации не принял пароль админки');
      /* A static host answers 404/405 for /api/publish: try the real backend. */
    ...
    throw new Error('Не удалось связаться с сервером публикации. Серверный адрес восстановлен автоматически — обновите страницу и повторите публикацию.');
  }
  ...
  window.getFile=async function(path){
    if(!useServer())return originalGetFile(path);
  ...
  if(card){
    ...
    const panel=document.createElement('div');panel.id='serverPublishPanel';panel.innerHTML=`
      <div class="field"><label>Сервер публикации</label><div class="token-row"><input id="publishApiUrl" placeholder="${SERVER_API}"><button type="button" class="btn green" id="savePublishApi">Сохранить</button></div><div class="hint">Публичный сайт работает статически, поэтому публикации отправляются через защищённый сервер ProVkus.</div></div>
    ...
    function paint(){
      const ok=useServer(),c=$('#serverConn');
      if(c){c.classList.toggle('ok',ok);c.querySelector('span').textContent=ok?'Сервер публикации готов · быстрый пакетный режим':'Сервер ещё не подключён'}
      if(stat)stat.textContent=ok?'SERVER':'—';
      const legacy=token?.closest('.field');if(legacy)legacy.style.display=ok?'none':'';
    }
    $('#savePublishApi').onclick=()=>{
      const v=clean(url.value);
      if(v&&!/^https:\/\//i.test(v)){flash?.('Нужен HTTPS-адрес сервера');return}
      if(v&&v!==SERVER_API&&!STATIC_ORIGINS.has(v))localStorage.setItem(API_KEY,v);else localStorage.removeItem(API_KEY);
      url.value=apiBase();paint();flash?.('Сервер публикации сохранён');
    };
    ...
    if(token?.closest('.field')){
      ...
      const details=document.createElement('details');details.id='legacyGithubAccess';
      const summary=document.createElement('summary');summary.textContent='Резервный способ: GitHub-токен';
      details.appendChild(summary);legacy.parentNode.insertBefore(details,legacy);details.appendChild(legacy);
    }
