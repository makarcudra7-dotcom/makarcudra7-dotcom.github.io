(()=>{
  ...
  function install(){
    installDedupe();
    const btn=$('#publishBtn');if(!btn||btn.dataset.fastPublishWrapped==='1'||typeof btn.onclick!=='function')return;
    ...
    btn.onclick=async function(e){
      ...
      try{
        ...
        if(result===true&&batched){
          const o=window.collect?.()||{},scheduled=new Date(o.publishedAt||0).getTime()>Date.now()+30000,label=scheduled?'Schedule':'Publish';
          await window.commitPublishBatch?.(`${label}: ${o.headline||o.slug||'material'}`);
          if(typeof flash==='function')flash(scheduled?'Материал поставлен в очередь':'Материал опубликован одним пакетом');
          window.renderPosts?.();
  ...
  let n=0,t=setInterval(()=>{n++;if(typeof window.beginPublishBatch==='function'&&typeof $('#publishBtn')?.onclick==='function'&&$('#placementCard')){clearInterval(t);install()}else if(n>240)clearInterval(t)},50)
})();
